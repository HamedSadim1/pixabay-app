...
export const useSearch = (): SearchState & SearchActions => {
  ...
  const updateURL = (query: string, page: number = 1) => {
    ...
    if (imageType !== "photo") params.set("type", imageType);
  ...
  const performSearch = useCallback(
    async (page: number = 1, append: boolean = false) => {
      ...
      try {
        ...
        // Add filters
        if (imageType !== "all") queryString += `&image_type=${imageType}`;
        if (orientation !== "all") queryString += `&orientation=${orientation}`;
        if (color !== "all") queryString += `&colors=${color}`;
        if (minWidth) queryString += `&min_width=${minWidth}`;
        if (minHeight) queryString += `&min_height=${minHeight}`;
    ...
    [search, imageType, orientation, color, minWidth, minHeight, results]
  );
